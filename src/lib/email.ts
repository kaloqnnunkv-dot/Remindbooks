...
export async function sendOrderStatusUpdate(d: {
  ...
  const COPY: Record<string, { subject: string; title: string; body: string }> = {
    PAID: isCod
      ? {
          // При наложен платеж пари още не са постъпили — клиентът плаща на
          // куриера. „Получихме плащането ви“ тук е чиста неистина.
          subject: `Поръчка ${d.orderNumber} е в обработка`,
          title: "Поръчката е в обработка",
          body: "Потвърдихме поръчката ви и започваме подготовката. Плащането е при получаване, директно на куриера.",
        }
      : {
    ...
    CANCELLED: {
      ...
      body: isCod
        ? "Поръчката ви беше отказана. Няма да бъдете таксувани — плащането беше предвидено при получаване. При въпроси, отговорете на този имейл."
        : "Поръчката ви беше отказана. Ако плащането е било извършено, сумата ще бъде възстановена. При въпроси, отговорете на този имейл.",
    },
    REFUNDED: {
